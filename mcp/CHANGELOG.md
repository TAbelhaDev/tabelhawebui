...
- Catálogo expõe o compound `Landing`: `Landing.Hero`/`Landing.Steps`/
  `Landing.Features`/`Landing.Roadmap`/`Landing.Footer` como sub-partes
  compostas (mesmo padrão de `Card.Header` e `Timeline.Item`).
